    ...
    where TId : IVogen<TId, int>
{
    ...
    {
        var matchingProperties = typeof(TContext)
            .GetProperties()
        ...
        if (matchingProperties.Count == 0)
        {
            throw new InvalidOperationException(
                $"No properties found in the EFCore context for a DBSet of {nameof(TEntityBase)}"
            );
        }
        ...
        {
            throw new InvalidOperationException(
                $"Multiple properties found in the EFCore context for a DBSet of {nameof(TEntityBase)}"
            );
        }
    ...
    public override TId Next(EntityEntry entry)
    {
        ...
        static int MaxFrom(IEnumerable<TEntityBase> es) => es.Any() ? es.Max(e => e.Id.Value) : 0;
    }
